...
def lucid(ct_path,output_seg_path,output_stdct_path=None,check=True,modelname=None,modelweight=None,output=None):
    ...
    from label import labelv2
    if output is None:
        labelnames = list(labelv2.values())
        output = np.arange(0,len(labelnames))
        output = [[ooo] for ooo in output]
    for idx, labels in enumerate(output):
        ...
        if isinstance(labels, list):
            ...
            for label in labels:
                mask |= (combined == label)
            new_pred[mask] = 1  # 使用 idx + 1 作为新的标签索引
        else:
            # 否则，labels 就是一个单独的标签
            new_pred[combined == labels] = 1
        new_pred = new_pred.numpy()
        # 创建SimpleITK图像
        sitk_image = sitk.GetImageFromArray(new_pred)
        # 设置方向和像素间距
        sitk_image.SetDirection(ct_itk.GetDirection())
        sitk_image.SetSpacing(ct_itk.GetSpacing())
        sitk_image.SetOrigin(ct_itk.GetOrigin())
        ...
        if not os.path.exists(output_seg_path):
            os.makedirs(output_seg_path)
            print(f"目录已创建：{output_seg_path}")
        sitk.WriteImage(sitk_image, os.path.join(output_seg_path,f"{labelname}.nii.gz"))
        print("create nii.gz ",os.path.join(output_seg_path,f"{labelname}.nii.gz"))